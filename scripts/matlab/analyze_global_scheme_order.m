...
mesh_0 = load_steady_state_solution(FILE, "mesh_12", 0);
mesh_1 = load_steady_state_solution(FILE, "mesh_23", 0);
...
first_order = 0.1*h.^(1.0);
second_order = 1.0*h.^(2.0);
third_order = 10.0*h.^(3.0);
fourth_order = 100.0*h.^(4.0);

hold on
loglog(h(2:end), E(2:end), '-bo', 'MarkerSize', 10, 'LineWidth', 2);
%loglog(h, E_p);
...
xlabel('h','interpreter', 'latex', 'FontSize', 36)
...
saveas(gcf,'convergence_global','svg');
